"""Pytest bootstrap for backend-domain tests."""
...
for path in (REPO_ROOT, BACKEND_ROOT, COMPUTE_SRC, SHARED_CONTRACTS_SRC):
    path_str = str(path)
...
def symlinks(tmp_path: Path) -> None:
    ...
    Duplicated in `mcp-server/tests/conftest.py`; the two suites install as
